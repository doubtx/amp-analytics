...
export default {
  ...
  async mounted () {
    if (this.$route.query.domain && this.$route.query.start && this.$route.query.end && this.$route.query.path) {
      this.fetchStats()
    }
  ...
  methods: {
    ...
    async handleDateChange(selectedDates) {
      ...
      if (selectedDates.length > 1) {
        ...
        console.log(start)
        console.log(end)
        this.$router.push({ path: '/page', query: { start: start, end: end, domain: this.$route.query.domain} })
        this.fetchStats()
      }
    }
  }
